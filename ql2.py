...
class QPlayer(Player):
    ...
    # x[0,31]+2  y[0,15]  pad[0,15] dx{-1,1}  [0,5]  dt[1]
    states = 98306 # 2 + 32*16*16*2*6
    ...
    # x[0,31]+2  y[0,15]  p[0,15]  dx{-1,1}  dy[0,5]  dt[1]
    @classmethod
    def flatten_grid_count(cls, x, y, p, dx, dy):
        ...
        return int(6*(2*(16*(16*x + y) + p) + dx) + dy)
    ...
    def state(self, table):
        ...
        if table.ball.x >= table.width + table.ball.r:
            return 2
        return 3 + self.flatten_grid_count(table.ball.x, table.ball.y,
    ...
    def alpha(self):
        return 10000 / (10000 + self.episodes())
...
class QEpsilonGreedyPlayer(QPlayer):
    ...
    def __init__(self, gamma, epsilon, position):
        super().__init__(gamma=gamma, epsilon=epsilon, decay=1, position=position)
