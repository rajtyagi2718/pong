...
class QLambda(QPlayer):
    ...
    def __init__(self, gamma, epsilon, lambda_, max_episodes, position):
...
class QLPlayer(Player):
    ...
    def state(self, table, position):
        ...
        if x < 2:
            ...
            return 2
        x -= 2
        return QEpsilonGreedyPlayer.flatten_grid_count(x, table.ball.y,
            getattr(table, position).centery, table.ball.dx, table.ball.dy)

    def strategy(self, table, position):
        s = self.state(table, position)
...
left_name = 'q-gamma-0.999-epsilon-0.999-decay-1-position-left_pad.npy'
right_name = 'q-gamma-0.999-epsilon-0.999-decay-1-position-right_pad.npy'
ql = QLPlayer(np.load(left_name), None)
...
def main():
    ...
    for lambda_ in (.5, .2, .9):
        for e in (.5, .9, .1):
            t = time.time()
            table = Table()
            ql = QLambda(gamma=g, epsilon=e, lambda_=lambda_, max_episodes=m,
                         position='left_pad')
            ai = AI()
